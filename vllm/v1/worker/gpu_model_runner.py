...
class GPUModelRunner(
    ...
    def _prepare_inputs(
        ...
        kv_positions = self.positions[:total_num_scheduled_tokens]
        ...
        if self.cache_config.active_kv_eviction_policy == "paged":
            ...
            active_num_computed = np.zeros(num_reqs, dtype=np.int64)
            block_counts = self.input_batch.block_table[0].num_blocks_per_row
            for req_idx in range(num_reqs):
                abs_num_computed = int(
                    self.input_batch.num_computed_tokens_cpu[req_idx]
                )
                active_blocks = int(block_counts[req_idx])
                if active_blocks == 0:
                    active_tokens = 0
                else:
                ...
                active_num_computed[req_idx] = active_tokens
                self.seq_lens[req_idx] = (
                    active_tokens + int(num_scheduled_tokens[req_idx])
                )

            kv_positions = torch.empty_like(
                self.positions[:total_num_scheduled_tokens]
            )
            for req_idx in range(num_reqs):
                ...
                kv_positions[start:end] = torch.arange(
                    active_num_computed[req_idx],
                    active_num_computed[req_idx] + end - start,
    ...
    def _compute_paged_eviction_block_scores(
        ...
        for req_id in req_ids:
            ...
            num_computed_tokens = previous_num_computed + num_scheduled_tokens
            prefill_len = int(
                self.input_batch.num_prompt_tokens_cpu_tensor[req_index]
            )
